...
└── ws/
    └── src/
        └── limo_ros2/  # upstream clone, with 8 fixes
```
...
required. To reproduce the environment from scratch:
...
If upstream moves and `git am` refuses, `git apply --3way` usually sorts it out.
...
1. `slam_toolbox` in online async mode to map the room.
